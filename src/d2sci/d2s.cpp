...
// i64 k, h, trim, add_ten;

// /* fast path */
// if(0)
// while (likely((sig_bin != 0x10000000000000ull)))// frac !=0
// {
...
// cb = sig_bin << (h + 1); /* h = 0/1/2/3 */ // [1,4]
// u128_mul(cb, p10_lo, &sig_hi, &sig_lo);
// u128_mul_add(cb, p10_hi, sig_hi, &sig_hi, &sig_lo);
// // p10*cb; u128*u64;
// one = sig_hi % 10 ;// last dec digit, need two mul
// ten = sig_hi - one;// (sig_hi / 10) * 10;

// /* right shift 4 to fit in u64 */
// c = (one << (64 - 4)) | (sig_lo >> 4);
// half_ulp = p10_hi >> (4 - h);
// t0 = ((u64)10 << (64 - 4));//const value
// t1 = c + half_ulp;
...
// //         //printf("%u\n",cnt++);//   38000/3.2e7 = 0.11875%
// //         break;
// //     }
// round_u1 = (sig_lo >= ((u64)1 << 63));
// //round_u0 = (t1 >> 60) >= 10;//t1 >= 10<<60
// round_u0 = (t1 >= t0 );//t1 >= 10<<60
// round_d0 = (half_ulp >= c);
...
// irregular = (sig_bin == 0x10000000000000ull);// frac == 0
...
// // unsigned long long tmp_high;
// // _mulx_u64(1844674407370955162ull, sig_hi, &tmp_high);
...
// // one = sig_hi % 10;//need two mul
// // ten = sig_hi - one;
// /* right shift 4 to fit in u64 */
// c = (one << (64 - 4)) | (sig_lo >> 4);
// half_ulp = p10_hi >> (4 - h);
// t0 = ((u64)10 << (64 - 4));// 10<<60 , const
// t1 = c + half_ulp;
// /* check range */
// //round_u1 = (sig_lo > ((u64)1 << 63));
...
// // if (!irregular)
// // {
// //     // round_u1 = (sig_lo >= ((u64)1 << 63));// lo >> 63;
// //     // if (sig_lo == ((u64)1 << 63))
// //     //     round_u1 = (sig_hi & 1);
// //     // round_d0 = (half_ulp >= c);
// //     // if (half_ulp == c)
...
// //round_u0 = (t1 >> 60) >= 10;
// //round_u0 = t1 >= (10ull<<60);
// round_u0 = t1 >= t0;
// //if (t0 - t1 <= (u64)1)
// // if (t0 - 1 <= t1)//t1 >= t0-1  t1 > t0 - 2
// // {
// //     if ( (k == 0) | (t0 - t1 == 1))//t1 == t0 - 1
// //         round_u0 = !(sig_bin & 1);//last bit is 0
// // }
// //int condition = (t1 >= t0 - 1 ) & ((k==0) | (t1 == t0-1)) ;
// //int condition = (t1==t0-1) | ( (t1 >= t0) & (k==0) );
// //unsigned int ku = *(unsigned int*)&k;//
// int condition = (t1 == t0 - 1) | ( round_u0 > *(u64*)&k );
// if( condition )
// {
// round_u0 = !(sig_bin & 1);
// }
// //round_u0 = (t1 >= t0 ) | (((t1 >= t0 - 1 ) & ((k==0) | (t1==t0-1)) ) > (sig_bin & 1)) ;
...
// /* round result */
// trim = (round_d0 | round_u0);//down or up 
// add_ten = (round_u0 ? 10 : 0);//round up
// dec_one = sig_hi + round_u1;// sig_hi or sig_hi + 1
// dec_ten = ten + add_ten;
// dec = trim ? dec_ten : dec_one;// sig_hi/10*10 or sig_hi/10*10+10
// gcc_load_barrier(dec_one); /* let gcc emit cmov */
// *sig_dec = dec;
// *exp_dec = k;
// }
...
// one = sig_hi - ten;
// c = (one << (64 - 4)) | (sig_lo >> 4);// (high , low) >> 4
// half_ulp = p10_hi >> (5 - h);
...
extern "C" int d2s_avx512(double v, char *buffer)
{
    //============ README start===========
    ...
    // 3. [1,1e9)  : 1.23e3   ->   123       ,  1.23e5   -> 12300       123.45     -> 123.45
    ...
    *(short *)(ptr + start_write_pos) = high1 | ('.' * 256 + '0');
    ...
    const ull *exp_ptr = &exp_result3[324];
    *(ull *)(ptr + 18 - flag + start_write_pos - ctz) = ((-7 <= e10) & (e10 <= 0)) ? 0 : exp_ptr[e10]; // remove tail zero
...
extern "C" int d2s_sse(double v, char *buffer)
{
    //============ README start===========
    ...
    // 3. [1,1e9)  : 1.23e3   ->   123       ,  1.23e5   -> 12300
    // 4. >=1e9    : 1.23e10  ->   1.23e+10  ,  1.23e9   -> 1.23e+09  , 1.23e103   -> 1.23e+103
...
#endif
    const ull *exp_ptr = &exp_result3[324];
    *(ull *)(ptr + 18 - flag + start_write_pos - ctz) = flag2 ? 0 : exp_ptr[e10]; // remove tail zero
    // when double value < 1e-309 ; equal digit_out < 1e15
    // also 5e-324 <= v < 1e-309 ;
#if 1
    if (digit_out < (ull)(1e15)) // [[unlikely]]
...
// extern "C" int d2s_yy_sse2(double v, char *buffer){
//     char* end_buf = yy_double_to_string(v,buffer);
//     *end_buf = '\0';
// }
...
extern "C" int d2s_yy_sse(double v, char *buffer)
{
    //============ README start===========
    // date : 2025.3.6
    // this code base on yy_double algorithm
    // sse implementation max use sse4.1
    // when input double is seq integer , may not efficient
    // I have limited time, so the code may have bugs and the performance may not be optimal.
    ...
    *(u64 *)buffer = *(u64 *)"0.000000"; // 8byte
    ...
    // result = digit_out * 10^k
#else
    unsigned long sig_dec;
    int32_t exp_dec;
    f64_bin_to_dec(c, q, &sig_dec, &exp_dec);
    ll k = exp_dec;
    u64 digit_out = sig_dec;
    //return digit_out + k;
#endif
